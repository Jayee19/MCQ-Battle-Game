...
const QuizRoomCard = () => {
  ...
  const handleFinish = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:5001/roomFriendGame/${roomId}`, {
        Finished : 1,
        removed : 0,
        playerIndex,
    ...
    if(FinishedResult === 2){
      ...
      if((ParticipatedData[0].correctAnswers > ParticipatedData[1].correctAnswers) || (ParticipatedData[0].correctAnswers === ParticipatedData[1].correctAnswers && ParticipatedData[0].totalTimeTaken < ParticipatedData[1].totalTimeTaken)){
          ...
          try{
            await axios.put(`http://localhost:5001/accuracyRoom/incrementCounts/${player1Email}`, {
                "type": "win",
                "subject": subject,
                "difficulty": difficulty
            });
    
            await axios.put(`http://localhost:5001/accuracyRoom/incrementCounts/${player2Email}`, {
              "type": "loss"
            });
          }catch(err){
            console.log(err);
  ...
  const handleClose = async () => {
    setShowResults(false);
      await axios.put(`http://localhost:5001/roomFriendGame/${roomId}`, {
        Finished : 0,
        removed : 1,
      });
